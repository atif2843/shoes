...
export default function ProductDetail({ product }) {
  ...
  const { cartItems, increaseQty, openCart } = useCartStore();
  ...
  const discount = product.originalPrice
    ? Math.round(
        ((product.originalPrice - product.sellPrice) / product.originalPrice) *
          100
      )
    : 0;
              ...
              {product.color &&
                product.color.map((color, idx) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-2 outline-3 ${
                      selectedColor === color
                        ? "outline-gray-500"
                        : "outline-gray-200"
                    }`}
          ...
          {isAddToCartDisabled && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
              <p className="text-sm text-yellow-700">
                Please select both a color and size to add this item to your
                cart
              </p>
            </div>
          )}
          ...
          <div className="mt-6 border-t pt-4">
            ...
            {isDetailsOpen && (
              <div className="mt-2 text-sm text-gray-700 bg-gray-200 p-4 rounded-md">
                ...
                <p className="my-3">Gender: {product.gender}</p>
